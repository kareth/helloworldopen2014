...
void BinaryThrottleScheduler::Optimize(const CarState& state) {
  double best_dist = 0;
  double best_throttle = 0; 
  int best_mask = -1;
  for (int i=0; i<=2; ++i) {
    double throttle = i/2.0;
    ...
    double distance;
    int mask = FindBestMask(next, &distance);
    //printf("%.1f ", distance);
    if (mask != -1) {
        if (this_dist + distance > best_dist) {
            best_dist = this_dist + distance;
  ...
  //printf("best=%d,%.2f,%.2f",best_mask,best_throttle,best_dist);
  if (best_mask == -1) {
      best_throttle = 0;
      best_mask = 0;
  }
  ...
  schedule_.clear();
  schedule_.push_back(best_throttle);
  for (int g = 0; g < groups_.size(); g++)
    for (int t = 0; t < groups_[g]; t++)
      schedule_.push_back((best_mask & (1 << g)) > 0);
...
int BinaryThrottleScheduler::FindBestMask(const CarState& state, double* distance, int from) {
  ...
  *distance = -1;
  ...
  for (int throttle = 1; throttle >= 0; throttle--) {
    bool fail = false;
...
//      break; // If found for 1, then no real reason to test 0
    }
  }
